...
def render_head(cfg):
    ...
    a("")
    a("    <!-- Google tag (gtag.js) — mọi trang trừ admin/index.html (SEO_EXCLUDED), xem")
    a("         check_coverage(). Bài viết CMS mới cũng có vì render_head() chạy lại cho")
    a("         từng bài trong ALL_POSTS mỗi lần build. -->")
    a('    <script async src="https://www.googletagmanager.com/gtag/js?id=G-7VBRBP2X4L"></script>')
    a("    <script>")
    a("      window.dataLayer = window.dataLayer || [];")
    a("      function gtag(){dataLayer.push(arguments);}")
    a("      gtag('js', new Date());")
    a("")
    a("      gtag('config', 'G-7VBRBP2X4L');")
    a("    </script>")
    a("")
    a("    <!-- Primary meta -->")
